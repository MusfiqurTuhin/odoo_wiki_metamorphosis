      ---
      technical_name: crm_claim
      display_name: "Claims Management"
      project_slug: precisiontex-v19-custom
      curated: false
      reusable: false
      marketing_summary: "Track your customers/vendors claims and grievances."
      odoo_version: "19.0"
      category: TBD
      git:
        repo_url: ""
        path: precisiontex-v19-custom/crm_claim
        branch: main
      contributors:
        - name: "Odoo S.A."
  role: author
- name: "Tecnativa"
  role: author
- name: "Odoo Community Association (OCA)"
  role: author
      depends: []
      used_in:
        - precisiontex-v19-custom
      versions:
        - version: "19.0.1.1.0"
          date: "2026-06-13"
          odoo_version: "19.0"
          whats_new: "Seeded from manifest scan. Update with release notes."
      ---

        # Claims Management

        `crm_claim`

        ## Module brief

        Track your customers/vendors claims and grievances.

        ## Who worked on this module

        | Name | Role |
        |------|------|
        | Odoo S.A. | author |
| Tecnativa | author |
| Odoo Community Association (OCA) | author |

        _Source: `__manifest__.py` author field. Confirm and extend via git history._

        ## Technical details

        | Field | Value |
        |-------|-------|
        | Technical name | `crm_claim` |
        | Current version | `19.0.1.1.0` |
        | Project | [Precisiontex](../index.md) |
        | Git path | `precisiontex-v19-custom/crm_claim` |
        | Manifest author | Odoo S.A., Tecnativa, Odoo Community Association (OCA) |

        ## Version history

        | Version | Date | What's new |
        |---------|------|------------|
        | 19.0.1.1.0 | 2026-06-13 | Seeded from scan — add release notes |

        !!! note "Needs review"
            This page was auto-generated. Set `curated: true` in front matter after manual review.
