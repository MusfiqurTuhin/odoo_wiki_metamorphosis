      ---
      technical_name: product_configurator
      display_name: "Product Configurator"
      project_slug: precisiontex-v19-custom
      curated: false
      reusable: false
      marketing_summary: "Base for product configuration interface modules"
      odoo_version: "19.0"
      category: TBD
      git:
        repo_url: ""
        path: precisiontex-v19-custom/product_configurator
        branch: main
      contributors:
        - name: "Pledra"
  role: author
- name: "Odoo Community Association (OCA)"
  role: author
      depends: []
      used_in:
        - precisiontex-v19-custom
      versions:
        - version: "19.0.1.4.0"
          date: "2026-06-13"
          odoo_version: "19.0"
          whats_new: "Seeded from manifest scan. Update with release notes."
      ---

        # Product Configurator

        `product_configurator`

        ## Module brief

        Base for product configuration interface modules

        ## Who worked on this module

        | Name | Role |
        |------|------|
        | Pledra | author |
| Odoo Community Association (OCA) | author |

        _Source: `__manifest__.py` author field. Confirm and extend via git history._

        ## Technical details

        | Field | Value |
        |-------|-------|
        | Technical name | `product_configurator` |
        | Current version | `19.0.1.4.0` |
        | Project | [Precisiontex](../index.md) |
        | Git path | `precisiontex-v19-custom/product_configurator` |
        | Manifest author | Pledra, Odoo Community Association (OCA) |

        ## Version history

        | Version | Date | What's new |
        |---------|------|------------|
        | 19.0.1.4.0 | 2026-06-13 | Seeded from scan — add release notes |

        !!! note "Needs review"
            This page was auto-generated. Set `curated: true` in front matter after manual review.
