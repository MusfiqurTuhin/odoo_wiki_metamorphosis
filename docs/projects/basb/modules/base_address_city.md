    ---
    technical_name: base_address_city
    display_name: "City Addresses"
    project_slug: basb
    curated: false
    reusable: false
    marketing_summary: "Add a many2one field city on addresses"
    odoo_version: "TBD"
    category: TBD
    git:
      repo_url: ""
      path: basb/base_address_city
      branch: main
    contributors:
      - name: "TBD"
role: author
    depends: []
    used_in:
      - basb
    versions:
      - version: "N/A"
        date: "2026-06-13"
        odoo_version: "TBD"
        whats_new: "Seeded from manifest scan. Update with release notes."
    ---

# City Addresses

`base_address_city`

## Module brief

Add a many2one field city on addresses

## Who worked on this module

| Name | Role |
|------|------|
| TBD | author |

_Source: `__manifest__.py` author field. Confirm and extend via git history._

## Technical details

| Field | Value |
|-------|-------|
| Technical name | `base_address_city` |
| Current version | `N/A` |
| Project | [Basb](../index.md) |
| Git path | `basb/base_address_city` |
| Manifest author | N/A |

## Version history

| Version | Date | What's new |
|---------|------|------------|
| N/A | 2026-06-13 | Seeded from scan — add release notes |

!!! note "Needs review"
    This page was auto-generated. Set `curated: true` in front matter after manual review.
