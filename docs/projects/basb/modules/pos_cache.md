    ---
    technical_name: pos_cache
    display_name: "pos_cache"
    project_slug: basb
    curated: false
    reusable: false
    marketing_summary: "Enable a cache on products for a lower POS loading time."
    odoo_version: "1.0"
    category: TBD
    git:
      repo_url: ""
      path: basb/pos_cache
      branch: main
    contributors:
      - name: "TBD"
role: author
    depends: []
    used_in:
      - basb
    versions:
      - version: "1.0"
        date: "2026-06-13"
        odoo_version: "1.0"
        whats_new: "Seeded from manifest scan. Update with release notes."
    ---

# pos_cache

`pos_cache`

## Module brief

Enable a cache on products for a lower POS loading time.

## Who worked on this module

| Name | Role |
|------|------|
| TBD | author |

_Source: `__manifest__.py` author field. Confirm and extend via git history._

## Technical details

| Field | Value |
|-------|-------|
| Technical name | `pos_cache` |
| Current version | `1.0` |
| Project | [Basb](../index.md) |
| Git path | `basb/pos_cache` |
| Manifest author | N/A |

## Version history

| Version | Date | What's new |
|---------|------|------------|
| 1.0 | 2026-06-13 | Seeded from scan — add release notes |

!!! note "Needs review"
    This page was auto-generated. Set `curated: true` in front matter after manual review.
