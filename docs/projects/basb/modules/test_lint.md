    ---
    technical_name: test_lint
    display_name: "test-lint"
    project_slug: basb
    curated: false
    reusable: false
    marketing_summary: "A module to test Odoo code with various linters."
    odoo_version: "TBD"
    category: TBD
    git:
      repo_url: ""
      path: basb/test_lint
      branch: main
    contributors:
      - name: "TBD"
role: author
    depends: []
    used_in:
      - basb
    versions:
      - version: "0.1"
        date: "2026-06-13"
        odoo_version: "TBD"
        whats_new: "Seeded from manifest scan. Update with release notes."
    ---

# test-lint

`test_lint`

## Module brief

A module to test Odoo code with various linters.

## Who worked on this module

| Name | Role |
|------|------|
| TBD | author |

_Source: `__manifest__.py` author field. Confirm and extend via git history._

## Technical details

| Field | Value |
|-------|-------|
| Technical name | `test_lint` |
| Current version | `0.1` |
| Project | [Basb](../index.md) |
| Git path | `basb/test_lint` |
| Manifest author | N/A |

## Version history

| Version | Date | What's new |
|---------|------|------------|
| 0.1 | 2026-06-13 | Seeded from scan — add release notes |

!!! note "Needs review"
    This page was auto-generated. Set `curated: true` in front matter after manual review.
